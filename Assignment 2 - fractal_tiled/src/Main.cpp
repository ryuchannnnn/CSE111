...
// Make a TSQueue<FractalMessage> queue;
typedef TSQueue<std::shared_ptr<FractalMessage>> threadQ;
...
void do_tile(EasyBMP::Image * img, float left, float top, float xsize, float ysize, float img_min_x, float img_min_y, float img_max_x, float img_max_y, float img_x_size, float img_y_size) {
    ...
    render_fractal(msg);
};  // this is producing fractal msg for us already

void producerFunc(threadQ *queue, EasyBMP::Image * img, float left, float top, float xsize, float ysize, float img_min_x, float img_min_y, float img_max_x, float img_max_y, float img_x_size, float img_y_size)
{
    std::shared_ptr<FractalMessage> msg = std::make_shared<FractalMessage>(img, left, top, xsize, ysize, img_min_x, img_min_y, img_max_x, img_max_y, img_x_size, img_y_size);
    queue->push(msg);   
}

// Look in depth at consumer_func from the queue example – make a new version of that which consumes FractalMessages from the queue
void consumerFunc(threadQ *queue)
{   
    while(true)
    {
        // pop here 
...
int main()
{
    ...
    // modify the nested for loop so it pushes instances of FractalMessage into the queue (instead of executing the std::async stuff)
    for (size_t i = 0; i < num_tiles_x; i++) 
    {
        ...
        producerFunc(&queue, &img, fractal_tile_min_x, fractal_tile_min_y, fractal_tile_max_x, fractal_tile_max_y, img_tile_min_x, img_tile_min_y, img_tile_max_x, img_tile_max_y, img_size_x, img_size_y);
    ...
    // finish making all the coordinates 
    
    // make threads after everything is pushed
    // spin up your n threads looking at the queue for messages using pop – and feeding the message to render_fractal - again the queue example has the
    // pattern for this, it’s just what is done with message that is different.

    for(size_t  i = 0; i < 8; i++)
    {
        std::shared_ptr<std::thread> thread = std::make_shared<std::thread>(consumerFunc, &queue);
        threads.push_back(thread);
        // std::cout << "I have made thread: " << i << std::endl;
    }
    // finish making all the threads 
    ...
    // wait till the TSQueue is empty before exiting the application, but again, the queue example has the pattern for that
    queue.abort();
